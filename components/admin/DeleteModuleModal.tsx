// components/admin/DeleteModuleModal.tsx
'use client';
...
export default function DeleteModuleModal({
    ...
    const fetchStats = async () => {
        ...
        try {
            const response = await fetch(`/api/admin/delete-module?moduleId=${module.id}`);
            if (response.ok) {
    ...
    const hasContent = stats && (stats.lessons > 0 || stats.contents > 0 || stats.studentProgress > 0);
                ...
                {loadingStats ? (
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            ...
                            {stats.studentProgress > 0 && (
                                <div className="flex flex-col items-center p-3 rounded-lg bg-rose-950/20 border border-rose-500/10">
                                    ...
                                    <span className="text-lg font-semibold text-rose-300">{stats.studentProgress}</span>
                                    <span className="text-xs text-rose-300/70">progressos</span>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="p-3 rounded-lg bg-gray-800/30 border border-gray-700/30">
                        <p className="text-sm text-gray-400 text-center">
                            Este módulo está vazio (sem aulas ou conteúdo).
                        </p>
                    </div>
                ) : null}

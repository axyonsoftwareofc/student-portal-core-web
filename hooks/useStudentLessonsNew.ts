...
export function useStudentLessonsNew(moduleId: string | null, studentId: string | null) {
    ...
    const fetchLessons = useCallback(async (): Promise<void> => {
        ...
        try {
            ...
            // 3. Buscar todos os conteúdos dessas aulas
            ...
            // 5. Calcular progresso por aula
            const completedContentIds = new Set(
                typedProgressData.map((p: ProgressFromDB) => p.content_id)
            );

            const lessonsWithProgress: LessonSummary[] = typedLessonsData.map((lesson: LessonFromDB) => {
            ...
            console.error('[useStudentLessonsNew] Erro:', err);
